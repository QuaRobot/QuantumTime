...
class ForecastExperiment(Experiment):
    ...
    def instance(self,
        ...
        deeptime = get_model('deeptime',
                          dim_size=train_set.data_x.shape[1],
                          datetime_feats=train_set.timestamps.shape[-1])
        deeptime = sum(p.numel() for p in deeptime.parameters() if p.requires_grad)
        print('parameters:%f4' % ((deeptime-num)/deeptime))
        ...
        print(test_metrics)
        np.save(join(self.root, 'result.npy'), {'val': val_metrics, 'test': test_metrics})
...
def train(model: nn.Module,
    ...
    for epoch in range(epochs):
        ...
        for it, data in enumerate(tqdm(train_loader)):
            ...
            if (it + 1) % 100 == 0:
                logging.info(f"epochs: {epoch + 1}, iters: {it + 1} | training loss: {loss.item():.4f}")
        scheduler.step()
    ...
    for it, data in enumerate(loader):
            ...
            if report_metrics:
                ...
                if save_path is not None:
                    inps.append(x.detach().cpu().numpy())
            else:
